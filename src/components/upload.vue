<template>
  <div class="container">
    ...
    <!-- <div>{{data.length}} -= 1{{cubeWidth}}</div>  -->
  ...
  methods: {
      ...
      while (cur < fileSize) {
        ...
        console.log(
          `切片${count}大小是${this.format(offset)} 实际大小是${this.format(
            chunk.size,
          )},耗时${time}秒，是30秒的${rate}倍，修正大小为${this.format(parseInt(offset / rate, 10))}`,
        );
        // 动态调整offset
    ...
    createProgressHandler(item) {
      return (e) => {
        ...
        item = {
          ...item,
          percentage: ((loaded / total).toFixed(2) - 0) * 100,
        };
      };
    },
    // 原生req封装
        ...
        const start = async () => {
          ...
          while (counter < len && max > 0) {
            ...
            const i = urls.findIndex(
              (v) => v.status === statusMap.wait || v.status === statusMap.error,
            ); // 等待或者error
            if (i !== -1) {
              console.log(i, 'start');
    ...
    async calculateHashIdle(chunks) {
      ...
      return new Promise((resolve) => {
        ...
        let count = 0;
        // 根据文件内容追加计算
        const appendToSpark = async (file) => new Promise((res) => {
        ...
        const workLoop = async (deadline) => {
          ...
          while (count < chunks.length && deadline.timeRemaining() > 1) {
            ...
            if (count < chunks.length) {
              // 计算中
              that.hashPercentage = Number(((100 * count) / chunks.length).toFixed(2));
    ...
    async calculateHashSample() {
      ...
      return new Promise((resolve) => {
        ...
        const { file } = this.container;
        // 文件大小
        const { size } = this.container.file;
